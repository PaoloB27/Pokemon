...
class Simulation:
    ...
    def random_battle(self, input_pokemon):
        ...
        sampled_pokemon = self.to_pokemon_character(sampled_pokemon)
        ...
        while True:
            ...
            curr_turn_info["Starter Damage Inflicted"] = input_pokemon.use_move(chosen_move, sampled_pokemon, self.type_effectiveness)
            ...
            curr_turn_info["Wild Damage Inflicted"] = sampled_pokemon.use_move(chosen_move, input_pokemon, self.type_effectiveness)
            data_all_turns.append(curr_turn_info)
    ...
    def run_simulation(self):
        ...
        At the beginning of each battle, a starter pokemon is selected uniformly at random.
        ...
        # starter pokemons
        starter_pokemons = self.pokemons[self.pokemons["name"].isin(self.starter_names)]

        # initialize the data collected for this simulation run to an empty list
        self.collected_data = []
        ...
        for j in tqdm(range(1, self.n_games + 1), desc=f"Running the Simulation", unit="game"):
            ...
            starter = self.to_pokemon_character(starter)

            # run n_battles battles before exiting the game
            for k in range(1, self.n_battles + 1):

                # run the battle and collect data
                wild_pokemon_name, wild_pokemon_level, outcome, n_turns, residual_HP, turns_data = self.random_battle(starter)
                ...
                self.collected_data.extend(turns_data)
