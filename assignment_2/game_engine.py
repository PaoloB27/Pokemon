...
class GameEngine:
    ...
    def __init__(self, starter_pokemons, wild_pokemons, moves):
        """
        ...
        - wild_pokemons: list of wild pokemons, represented as dictionaries.
        - moves: list of moves, represented as dictionaries.
        """

        # set moves, starter pokemons and wild pokemons
        type_text("\nStarting the game...\n")
        self.moves = moves
        self.starter_pokemons = starter_pokemons
        self.wild_pokemons = wild_pokemons

        # initialize the pokemon trainer
        clear_terminal()
        self.pokemon_trainer = self.initialize_pokemon_trainer()
    ...
    def initialize_pokemon_trainer(self):
        """
        Initializes a PokemonTrainer with:
        - a name chosen by the user;
        - a starter pokemon chosen by the user;
        ...
        starter_pokemon_choice = choose_option([pokemon["name"] for pokemon in self.starter_pokemons], f"Ok {trainer_name}, I want you to choose one of the following pokemons:")
        clear_terminal()

        # create the starter pokemon and add it to the pokemon trainer's list
        chosen_pokemon = self.starter_pokemons[int(starter_pokemon_choice)]
    ...
    def attack(self, opponent_pokemon):
        ...
        options = [f"{move_name.capitalize()} | {pp} PP" for move_name, pp in self.pokemon_trainer.active_pokemon.curr_pps.items() if pp > 0]
        ...
        self.pokemon_trainer.active_pokemon.use_move(chosen_move, opponent_pokemon)
    ...
    def use_item(self, opponent_pokemon):
        ...
        options = [f"{item_name.capitalize()} | {quantity}" for item_name, quantity in self.pokemon_trainer.items.items()]
    ...
    def run_away(self, opponent_pokemon):
        ...
        type_text(f"{opponent_pokemon.name.capitalize()} prevents {self.pokemon_trainer.name.capitalize()} from running away, the battle continues!\n")
        return False
    ...
    def change_pokemon(self):
        ...
        available_pokemon = [pokemon for pokemon in self.pokemon_trainer.pokemon_list if pokemon is not self.pokemon_trainer.active_pokemon and pokemon.curr_hp > 0]
        available_pokemon_to_display = [f"{pokemon.name.capitalize()} | {pokemon.curr_hp} HP" for pokemon in self.pokemon_trainer.pokemon_list if pokemon is not self.pokemon_trainer.active_pokemon and pokemon.curr_hp > 0]
    ...
    def opponent_pokemon_turn(self, opponent_pokemon):
        ...
        opponent_pokemon.use_move(random.choice([move_name for move_name, pp in opponent_pokemon.curr_pps.items() if pp > 0]), self.pokemon_trainer.active_pokemon)
        ...
        if self.pokemon_trainer.active_pokemon.curr_hp <= 0:
            ...
            if not self.change_pokemon():
                type_text(f"\nAll {self.pokemon_trainer.name.capitalize()}'s pokemon are K.O., so {self.pokemon_trainer.name.capitalize()} loses the battle!\n")
                return True
    ...
    def check_pps(self, opponent_pokemon):
        ...
        type_text(f"\n{self.pokemon_trainer.name.capitalize()} has no pokemon with pp > 0 that can fight!\n")
    ...
    def battle(self, opponent_pokemon):
        ...
        while True:
            ...
            type_text(f"\nRound {round}\n")
            type_text(f"\n{self.pokemon_trainer.active_pokemon.name.capitalize()} HP: {self.pokemon_trainer.active_pokemon.curr_hp}\n")
            type_text(f"{opponent_pokemon.name.capitalize()} HP: {opponent_pokemon.curr_hp}\n")
    ...
    def pokemon_store_action(self):
        ...
        for item in store_items:
            curr_items_quantities[item] = self.pokemon_trainer.items.get(item, 0)    # set the value to 0 if the item is not in self.   trainer.items
